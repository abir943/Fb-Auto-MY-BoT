...
module.exports = {
    ...
    version: "1.0.1",
    usePrefix: true,
    description: "Sends a random 6-pack image to boost your energy.",
    
    execute: async (api, event, args) => {
        const { threadID } = event;
        const cacheDir = path.join(__dirname, "cache");
        const filePath = path.join(cacheDir, "6pack.jpg");
        ...
        console.log(chalk.cyan("[6pack]") + " Fetching image...");
        ...
        request(encodeURI(selected))
            ...
            .on("close", () => {
                api.sendMessage({
                    ...
                    console.log(chalk.green("[6pack]") + " Image sent and cache cleaned.");
                });
            })
            .on("error", (err) => {
                console.log(chalk.red("[6pack] Failed to fetch image:"), err);
                api.sendMessage("❌ Failed to fetch image. Try again later.", threadID);
            });
    }
};
